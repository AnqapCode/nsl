...
export async function getAllUrls(options: GetAllUrlsOptions = {}): Promise<ApiResponse<{ urls: UrlWithUser[]; total: number }>> {
  try {
    ...
    if (filter !== "all") {
      transformedUrls = transformedUrls.filter((url) => {
        ...
        if (filter === "security" && url.flagReason) {
          return url.flagReason.toLowerCase().includes("security") || url.flagReason.toLowerCase().includes("phishing") || url.flagReason.toLowerCase().includes("malware");
        }

        if (filter === "inappropriate" && url.flagReason) {
          return url.flagReason.toLowerCase().includes("inappropriate") || url.flagReason.toLowerCase().includes("adult") || url.flagReason.toLowerCase().includes("offensive");
        }
    ...
    // apply pagination
    const paginateUrls = transformedUrls.slice(offset, offset + limit);
