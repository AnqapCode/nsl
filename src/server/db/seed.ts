...
import { urls, userRoleEnum, users } from "./schema";
import { db } from ".";
  ...
  try {
    ...
    const testUsers = [
      ...
      },
      {
        ...
        role: userRoleEnum.enumValues[1], // "user"
        createdAt: new Date(),
        updatedAt: new Date(),
      },
    ];
    ...
    const testUrls = [
      ...
      },
      {
        ...
        shortCode: "google",
        createdAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000), // 1 days ago
    ...
    for (const url of testUrls) {
      try {
        ...
        console.log(`❌ Error creating URL ${url.shortCode}:`, error);
      }
    }
    ...
    for (let i = 0; i < 10; i++) {
      const randomUrl = {
        ...
        userId: Math.random() > 0.5 ? insertedUsers[Math.floor(Math.random() * insertedUsers.length)].id : null,
      };

      try {
        await db.insert(urls).values(randomUrl);
        console.log(`✅ Created URL: ${randomUrl.shortCode} -> ${randomUrl.originalUrl}`);
      } catch (error) {
        console.log(`❌ Error creating random URL:`, error);
      }
    }
