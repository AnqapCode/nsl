...
export default async function FlaggedURLsPage({
  ...
  const urls = response.success && response.data ? response.data.urls.filter((url) => url.flagged) : [];
  ...
  const categorizedUrls = urls.reduce((acc, url) => {
    ...
    const category = reason.toLowerCase().includes("inappropriate")
      ? "inappropriate"
      : reason.toLowerCase().includes("security") || reason.toLowerCase().includes("malware") || reason.toLowerCase().includes("phishing") || reason.toLowerCase().includes("suspicious")
      ? "security"
      : "other";
        ...
        <div>
          <Card className="shadow-sm border-green-200 dark:border-green-800">
            <CardContent className="pt-8 pb-8 flex flex-col items-center justify-center">
              ...
              <p className="text-muted-foreground text-center max-w-md">All flagged URLs have been reviewed and are safe to use.</p>
            </CardContent>
          </Card>
        </div>
      ) : (
        <div className="grid gap-8">
          <Card className="shadow-sm border-yellow-200 dark:border-yellow-800">
            <CardHeader className="border-b px-3 py-1 border-yellow-100 dark:border-yellow-900/50 bg-yellow-50/50 dark:bg-yellow-900/20">
              ...
              <CardDescription>These URLs have been automatically flagged by our AI system as potentially unsafe or inappropriate. Please review each URL carefully before taking action.</CardDescription>
            </CardHeader>
            ...
            <CardContent className="pt-6">
              {categorizedUrls.security && categorizedUrls.security.length > 0 && (
                <div className="mb-8">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-lg font-medium text-red-700 dark:text-red-400">Security Concerns</h3>
                  </div>

                  <Card className="border-red-200 dark:border-red-900/50 bg-red-50/30 dark:bg-red-900/10 shadow-sm mb-6">
                    <CardContent className="pt-4">
                      <p className="text-sm text-red-700 dark:text-red-400 mb-2">These URLs have been flagged due to security concerns. Please review each URL carefully before taking action</p>
                    </CardContent>
                  </Card>
                  <UrlsTable urls={categorizedUrls.security} total={categorizedUrls.security.length} currentPage={page} currentSearch={search} currentSortBy={sortBy} currentSortOrder={sortOrder} highlightStyle="security" />
                </div>
              )}
              ...
              {categorizedUrls.inappropriate && categorizedUrls.inappropriate.length > 0 && (
                <div className="mb-8">
                  ...
                  <Card className="border-red-200 dark:border-red-900/50 bg-red-50/30 dark:bg-red-900/10 shadow-sm mb-6">
                    <CardContent className="pt-4">
                      <p className="text-sm text-red-700 dark:text-red-400 mb-2">These URLs have been flagged due to inappropriate content. Please review each URL carefully before taking action.</p>
                    </CardContent>
                  </Card>
                  <UrlsTable urls={categorizedUrls.inappropriate} total={categorizedUrls.inappropriate.length} currentPage={page} currentSearch={search} currentSortBy={sortBy} currentSortOrder={sortOrder} highlightStyle="inappropriate" />
                </div>
              )}
              ...
              {categorizedUrls.other && categorizedUrls.other.length > 0 && (
                <div className="mb-8">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-lg font-medium text-yellow-700 dark:text-yellow-400">Other Concerns</h3>
                  </div>

                  <Card className="border-yellow-200 dark:border-yellow-900/50 bg-yellow-50/30 dark:bg-yellow-900/10 shadow-sm mb-6">
                    <CardContent className="pt-4">
                      <p className="text-sm text-yellow-700 dark:text-yellow-400 mb-2">These URLs have been flagged due to inappropriate content. Please review each URL carefully before taking action.</p>
                    </CardContent>
                  </Card>
                  <UrlsTable urls={categorizedUrls.other} total={categorizedUrls.other.length} currentPage={page} currentSearch={search} currentSortBy={sortBy} currentSortOrder={sortOrder} highlightStyle="other" />
                </div>
              )}
            </CardContent>
          </Card>
