...
export function UserUrlsTable({ urls }: UserUrlsTableProps) {
  ...
  const copyToClipboard = async (shortCode: string) => {
    ...
    try {
      ...
      toast.success("Short URL copied to clipboard", {
        description: `The short URL "${shortUrl}" has been copied to your clipboard`,
      });
    } catch (error) {
  ...
  const handleEditSuccess = (newShortCode: string) => {
    ...
    setLocalUrls((prev) => prev.map((url) => (url.id === urlToEdit.id ? { ...url, shortCode: newShortCode } : url)));
  };

  if (localUrls.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-muted-foreground">You haven&apos;n created any short URLs yet. Click the button below to create your first short URL.</p>
      </div>
    );
  }
  ...
  return (
    <>
                ...
                <tr key={url.id} className="border-b hover:bg-muted/50">
                  <td className="py-3 px-4">
                    <div className="flex items-center">
                      <div className="max-w-xs truncate" title={url.originalUrl}>
                        {url.originalUrl.length > 20 ? `${url.originalUrl.substring(0, 20)}...` : url.originalUrl}
                      </div>
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center">
                      ...
                      <Button variant={"ghost"} size={"icon"} onClick={() => copyToClipboard(url.shortCode)} className="ml-2 size-8 cursor-pointer">
                        <Copy className="size-4" />
                      </Button>
                    </div>
                  </td>

                  <td className="py-3 px-4 text-muted-foreground">{url.clicks}</td>
                  ...
                  <td className="py-3 px-4 text-right">
                    <div className="flex justify-end">
                      <Button variant={"ghost"} size={"icon"} onClick={() => showQrCode(url.shortCode)} className="size-8 text-primary cursor-pointer hover:text-primary">
                        <QrCode className="size-4" />
                      </Button>

                      <Button variant={"ghost"} size={"icon"} onClick={() => handleEdit(url.id, url.shortCode)} className="size-8 text-primary cursor-pointer hover:text-primary">
                        <Edit className="size-4" />
                      </Button>

                      <Button variant={"ghost"} size={"icon"} onClick={() => handleDelete(url.id)} disabled={isDeleting === url.id} className="size-8 text-destructive cursor-pointer hover:text-destructive">
                        {isDeleting === url.id ? <div className="size-4 animate-spin rounded-full border-2 border-current border-t-transparent" /> : <Trash2Icon className="size-4" />}
                      </Button>
                    </div>
                  </td>
                </tr>
      ...
      {urlToEdit && <EditUrlModal isOpen={isEditModalOpen} onOpenChange={setIsEditModalOpen} urlId={urlToEdit.id} currentShortCode={urlToEdit.shortCode} onSuccess={handleEditSuccess} />}
    </>
  );
}
