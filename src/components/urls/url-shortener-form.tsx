...
const UrlShortenerForm = () => {
  ...
  const onSubmit = async (data: UrlFormData) => {
    ...
    try {
      ...
      // if a custom code is provided, appen it to the form
                ...
                <FormItem>
                  <FormControl>
                    <div className="flex items-center">
                      <span className="text-sm text-muted-foreground mr-2">{process.env.NEXT_PUBLIC_APP_URL || window.location.origin}/r/</span>
                      <Input placeholder="Custom code (optional)" {...field} value={field.value || ""} onChange={(e) => field.onChange(e.target.value || "")} disabled={isLoading} className="flex-1" />
                    </div>
                  </FormControl>
                  <FormMessage />
                </FormItem>
              ...
              <Card>
                <CardContent className="p-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button type="button" variant={"outline"} className="flex-shrink-0" onClick={copyToClipboard}>
                      <Copy className="size-4 mr-1" />
                      Copy
                    </Button>

                    <Button type="button" variant={"outline"} className="flex-shrink-0" onClick={showQrCode}>
                    ...
                    <div className="mt-3 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-md">
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-yellow-700 dark:text-yellow-400 mt-1">{flaggedInfo.message || "This URL will be reviewed by an administrator before it becomes fully active."}</p>
                          {flaggedInfo.reason && (
                            <p className="text-sm mt-2 text-yellow-600 dark:text-yellow-400">
                              <span className="font-medium">Reason:</span> {flaggedInfo.reason || "Unknown reason"}
                            </p>
                          )}
                        </div>
                      </div>
