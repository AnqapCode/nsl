...
export function UrlsTable({ urls, total, currentPage, currentSearch, currentSortBy, currentSortOrder, highlightStyle }: UrlsTableProps) {
  ...
  const getPaginationItems = () => {
    ...
    items.push(
      <PaginationItem key={"first"}>
        <PaginationLink href={`${basePath}?page=1${currentSearch ? `&search=${currentSearch}` : ""}${currentSortBy ? `&sortBy=${currentSortBy}&sortOrder=${currentSortOrder}` : ""}${additionalParams}`} isActive={currentPage === 1}>
          1
        </PaginationLink>
      </PaginationItem>
    );
    ...
    for (let i = Math.max(2, currentPage - 1); i <= Math.min(totalPage - 1, currentPage + 1); i++) {
      ...
      items.push(
        <PaginationItem key={i}>
          <PaginationLink href={`${basePath}?page=${i}${currentSearch ? `&search=${currentSearch}` : ""}${currentSortBy ? `&sortBy=${currentSortBy}&sortOrder=${currentSortOrder}` : ""}${additionalParams}`} isActive={currentPage === i}>
            {i}
          </PaginationLink>
        </PaginationItem>
      );
    ...
    if (totalPage > 1) {
      items.push(
        <PaginationItem key={"last"}>
          <PaginationLink
            href={`${basePath}?page=${totalPage}${currentSearch ? `&search=${currentSearch}` : ""}${currentSortBy ? `&sortBy=${currentSortBy}&sortOrder=${currentSortOrder}` : ""}${additionalParams}`}
  ...
  const handleManageFlaggedUrl = async (urlId: number, action: "approve" | "delete") => {
    try {
      setActionLoading(urlId);

      const response = await manageFlaggedUrl(urlId, action);
      if (response.success) {
        toast.success(action === "approve" ? "URL approved successfully." : "URL deleted successfully.");
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-md border">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="w-[300px]">
                <button className="flex items-center font-medium" onClick={() => handleSort("originalUrl")}>
                  Original URL
                  {getShortIcon("originalUrl")}
                </button>
              </TableHead>

              <TableHead className="w-[150px]">
                <button className="flex items-center font-medium" onClick={() => handleSort("shortCode")}>
                  Short Code
                  {getShortIcon("shortCode")}
                </button>
              </TableHead>

              <TableHead className="w-[100px]">
                <button className="flex items-center font-medium" onClick={() => handleSort("clicks")}>
                  Clicks
                  {getShortIcon("clicks")}
                </button>
              </TableHead>
              ...
              <TableHead className="w-[150px]">
                <button className="flex items-center font-medium" onClick={() => handleSort("userName")}>
                  Created By
                  {getShortIcon("userName")}
                </button>
              </TableHead>
          ...
          <TableBody>
            {urls.length === 0 ? (
              <TableRow>
                <TableCell>{currentSearch ? "No URLs found with the search term." : "No URLs found"}</TableCell>
              </TableRow>
            ) : (
              urls.map((url) => (
                <TableRow key={url.id} className={getHighlightStyles(url)}>
                  <TableCell className="font-medium">
                    <div className="flex items-center gap-2">
                      {url.flagged && (
                        <div className={getFlagIconColor()} title={url.flagReason || "Flagged By AI"}>
                          <AlertTriangle className="size-4" />
                        </div>
                      )}
                      <a href={url.originalUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline flex items-center gap-1 max-w-[250px] truncate">
                    ...
                    {url.userId ? (
                      <div className="flex items-center gap-2">
                        <Avatar className="size-6">
                          ...
                          <AvatarFallback className="text-xs">{url.userName?.substring(0, 2)}</AvatarFallback>
                        </Avatar>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        {url.flagged && (
                          <>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem className="text-green-600 dark:text-green-400" onClick={() => handleManageFlaggedUrl(url.id, "approve")}>
                              {actionLoading === url.id && <Loader2 className="size-4 mr-1" />}
                              <CheckCircle className="size-4 mr-1 text-green-700" />
                              Approve URL
                            </DropdownMenuItem>
                            <DropdownMenuItem className="text-red-600 dark:text-red-400" onClick={() => handleManageFlaggedUrl(url.id, "delete")}>
                              {actionLoading === url.id && <Loader2 className="size-4 mr-1" />}
      ...
      {totalPage > 1 && (
        <Pagination>
          <PaginationContent>
            <PaginationItem>
              <PaginationPrevious
                href={`${basePath}?page=${Math.max(1, currentPage - 1)}${currentSearch ? `&search=${currentSearch}` : ""}${currentSortBy ? `&sortBy=${currentSortBy}&sortOrder=${currentSortOrder}` : ""}${preserveParams()}`}
              />
            </PaginationItem>
            ...
            <PaginationItem>
              <PaginationNext
                href={`${basePath}?page=${Math.min(totalPage, currentPage + 1)}${currentSearch ? `&search=${currentSearch}` : ""}${currentSortBy ? `&sortBy=${currentSortBy}&sortOrder=${currentSortOrder}` : ""}${preserveParams()}`}
              />
            </PaginationItem>
          </PaginationContent>
        </Pagination>
      ...
      <div className="text-xs text-muted-foreground">
        ...
        {currentSearch && ` Search result for "${currentSearch}"`}
      </div>
    </div>
  );
}
